...
<p><spring:message code="book.overdue"/>:</p>
...
<table border="1">
	...
	<td width="200"><spring:message code="blacklist"/></td>
	</tr>
	...
	<c:forEach var="reading" items="${orders}">
		...
		<td width="300">
			...
			<c:out value="${reading.user.email}"/><br>
			ID: <c:out value="${reading.user.user_id}"/>
		</td>
													
		<td width="200"><c:out value="${reading.status}"/></td>
		<td width="200"><c:out value="${reading.dateOn}"/></td>
		<td width="200"><c:out value="${reading.dateOff}"/></td>
		
		<td width="200">
			[<a href="/web/admin/give_take/blacklist/${reading.user.user_id}/${reading.inventory.inventory_id}">
			<spring:message code="blacklist"/></a>]
		</td>
		</tr>
	</c:forEach>
</table>
...
<br><br>
    ...
    <table>
        <tr>
		    ...
		    <c:if test="${page != 1}">
		        <td>[<a href="/web/books/orders/overDate/${page - 1}"><spring:message code="previous"/></a>]</td>
		    </c:if>
		        ...
		        <td>
		            <c:forEach begin="1" end="${totPages}" var="i">
		                <c:choose>
		                    ...
		                    <c:otherwise>
		                        <td><a href="/web/books/orders/overDate/${i}">${i}</a></td>
		                    </c:otherwise>
		                </c:choose>
		            </c:forEach>
		        </td>
    		...
    		<c:if test="${totPages > 1  and page < totPages}">
		        <td>[<a href="/web/books/orders/overDate/${page + 1}"><spring:message code="next"/></a>]</td>
		    </c:if>  
        </tr>
    </table>