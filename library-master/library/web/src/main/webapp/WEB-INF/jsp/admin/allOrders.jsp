...
<table border="1">
	...
	<tr>
		...
		<td width="150"><spring:message code="give.status"/></td>
		<td width="150"><spring:message code="order.date"/></td>
	...
	<c:forEach var="order" items="${orders}">
		...
		<td width="150"><c:out value="${order.status}"/></td>
		<td width="150"><c:out value="${order.dateOrder}"/></td>
		
		<td width="300">
			<c:out value="${order.user.name}"/><br>
			<c:out value="${order.user.secondName}"/><br>
			<c:out value="${order.user.email}"/><br>
			ID: <c:out value="${order.user.user_id}"/>
		</td>
    ...
    <table>
        <tr>
		    ...
		    <c:if test="${page != 1}">
		        <td>[<a href="/web/books/orders/all/${page - 1}"><spring:message code="previous"/></a>]</td>
		    </c:if>
		        ...
		        <td>
		            <c:forEach begin="1" end="${totPages}" var="i">
		                <c:choose>
		                    ...
		                    <c:otherwise>
		                        <td><a href="/web/books/orders/all/${i}">${i}</a></td>
		                    </c:otherwise>
		                </c:choose>
		            </c:forEach>
		        </td>
    		...
    		<c:if test="${totPages > 1  and page < totPages}">
		        <td>[<a href="/web/books/orders/all/${page + 1}"><spring:message code="next"/></a>]</td>
		    </c:if>  
        </tr>
    </table>
</div>